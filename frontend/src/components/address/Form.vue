        ...
        <ul style="background-color: transparent">
          ...
          <li class="add-address-card">
            <div class="card-box" id="idcard0" @click="selectImage('idcard0')">
              <template v-if="form.idcardFrontUrl!=''">
                <img v-bind:src="fdshost+form.idcardFrontUrl" style="width: 100%;height: 100%;"/>
              </template>
              <template v-else>
            ...
            <div class="card-box" id="idcard1" @click="selectImage('idcard1')">
              <template v-if="form.idcardVersoUrl!=''">
                <img v-bind:src="fdshost+form.idcardVersoUrl" style="width: 100%;height: 100%;"/>
              </template>
              <template v-else>
                <div class="card-icon"></div>
  ...
  export default {
    ...
    methods: {
      ...
      uploadImage(){
        ...
        /*service.form("/addressManage/imgFileUpload", new FormData(form)).then((res) => {
         let ret = res.data
         if (ret.success) {
         let data = ret.data;
         this.fdshost = data["fdshost"];
      ...
      createPicker(){
        ...
        this.getRegionData(1).then((res) => {
          ...
          picker.on("picker.select", (arrVal, arrIndex) => {
            ...
            that.form.city = city[arrIndex[1]].text;
            that.form.country = country[arrIndex[2]].text
