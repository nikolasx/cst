  ...
  <div class="page order-container page-current">
    ...
    <div class="content">
      ...
      <div class="address-box">
        ...
        <a class="address-box-content" id="add-address">
          <div class="address-info-container">

            <template v-if="address==null">
              <a @click.prevent="selectAddress">
                <div class="address-label">
                  <i class="iconfont icon-dizhi"></i>
                  <span id="curAddress">添加地址</span>
                </div>
              </a>
            </template>
            <template v-else>
              <a class="address-detail-container" @click.prevent="selectAddress">
                <div class="address-detail">
                  <div class="address-detail-row">
                                <span class="detail-name"><i
                                  class="iconfont icon-touxiang"></i><label>{{address.consignee}}</label></span>
                    <span class="detail-phone"><i
                      class="iconfont icon-dianhua"></i><label>{{address.mobile}}</label></span>
                  </div>
                  <div class="address-detail-row">
                    <span><i
                      class="iconfont icon-dizhi"></i>{{address.province}}{{address.city}}{{address.country}}{{address.address}}</span>
                  </div>
                </div>
              </a>
            </template>
      ...
      <div class="tpl-container">
        <div class="total-order" v-if="baseInfo.quantity">
          <div class="total-order-label">共<label class="total-order-count">{{baseInfo.quantity}}</label>件</div>
        ...
        <ul class="order-list-container">
          ...
          <li class="order-item" v-for="item in list">
            ...
            <div class="order-item-info">
              ...
              <!--<div class="order-info-item order-info-price">CNY {{item.finalPrice}}</div>-->
              <div class="order-info-item-box">
                <div class="order-info-item-line">
                  <div class="order-info-item order-info-size">单价：<label class="cred">CNY {{item.finalPrice && item.finalPrice.toFixed(2)}}</label></div>
                </div>
                <div class="order-info-item-line">
                  <div class="order-info-item order-info-size">运费：<label class="cred">CNY {{item.freightPayable && item.freightPayable.toFixed(2)}}</label></div>
                  <div class="order-info-item order-info-size">税费：<label class="cred">CNY {{item.inlandTaxes && item.inlandTaxes.toFixed(2)}}</label></div>
                </div>
              </div>
            </div>
        ...
        <div class="real-amount" v-if="baseInfo.amountPayable">
          ...
          <div class="real-amount-content">
            <div class="real-amount-line"><strong>+CNY <label>{{baseInfo.amountPayable}}</label></strong></div>
